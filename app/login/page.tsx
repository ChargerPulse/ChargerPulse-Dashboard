...
export default function LoginPage() {
  ...
  const handleSubmit = async () => {
    if (!email || !password) {
      setStatus('error'); setMessage('Email and password are required.'); return
    }
    setStatus('loading')
    ...
    if (mode === 'signup') {
      ...
      else { setStatus('success'); setMessage('Account created! Check your email to confirm, then log in.') }
    } else {
        ...
        <div className="card" style={{ padding: 32 }}>
          ...
          {status === 'error' && (
            <div style={{ background: 'rgba(255,68,68,0.08)', border: '1px solid rgba(255,68,68,0.3)', borderRadius: 10, padding: '12px 16px', marginBottom: 16 }}>
              <p style={{ color: '#ff4444', fontSize: 13 }}>Error: {message}</p>
            </div>
          )}
          {status === 'success' && (
          ...
          <button onClick={handleSubmit} style={{ width: '100%', padding: 14, borderRadius: 10, fontSize: 15, fontWeight: 800, cursor: 'pointer', border: 'none', background: 'linear-gradient(135deg, #00d4ff, #a855f7)', color: 'white', marginTop: 8 }}>
            {status === 'loading' ? 'Please wait...' : mode === 'login' ? 'Sign In' : 'Create Account'}
          </button>
          <div style={{ textAlign: 'center', marginTop: 20 }}>
            <button onClick={() => { setMode(mode === 'login' ? 'signup' : 'login'); setStatus('idle') }} style={{ background: 'none', border: 'none', color: '#00d4ff', cursor: 'pointer', fontSize: 13 }}>
              {mode === 'login' ? "Don't have an account? Sign up" : 'Already have an account? Sign in'}
            </button>
          </div>
        </div>
        <p style={{ textAlign: 'center', color: '#334155', fontSize: 12, marginTop: 24 }}>Secured by Supabase Auth</p>
      </div>
    </div>
  )
}
